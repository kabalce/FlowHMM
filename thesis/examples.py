...
B = np.array([[0.5, 0.15, 0.15, 0.2], [0.2, 0.6, 0.1, 0.1], [0.1, 0.2, 0.2, 0.5]])
...
print(
    ", ".join(
        ["\\textcolor{" + colors_[c - 1] + "}{" + v + "}" for c, v in zip(res, res2)]
    )
)
...
Sigma = np.array(
    [[[1, -0.4], [-0.4, 0.8]], [[0.6, -0.5], [-0.5, 1.2]], [[0.9, 0.6], [0.6, 1.7]]]
)
...
lls = np.concatenate(
    [
        scipy.stats.multivariate_normal(mu[i], Sigma[i]).pdf(data).reshape(-1, 1)
        for i in range(3)
    ],
    axis=1,
)
...
plt.plot([r[0] for r in res3], [r[1] for r in res3], linestyle="-", marker="o", lw=0.3)
for i in range(16):
    x = np.random.uniform(-0.2, 0.2) if i not in [7] else -0.2
    y = 0.2 if i not in [14, 9, 13] else -0.2
...
print(
    ",\n ".join(
        [
            ...
            + colors_[c - 1]
            + "}{\\left(\\begin{array}{c} "
            + str("\\\\".join([str(w) for w in v.round(3)]))
            + "\\end{array}\\right)}"
            for c, v in zip(res, res3)
        ]
    )
)
...
a2l.to_ltx(
    np.array([[1, 2, 1, 0], [0, 2, 3, 0], [2, 2, 0, 1], [0, 0, 1, 0]]) / 15,
    frmt="{:6.4f}",
    arraytype="array",
)
...
a2l.to_ltx(
    np.array([2, 3, 9]).reshape(-1, 1) * A / 14, frmt="{:6.4f}", arraytype="array"
)
