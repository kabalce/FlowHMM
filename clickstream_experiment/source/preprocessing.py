...
def load_raw_clickstream():
    with open(f'{PROJECT_PATH}/clickstream_experiment/data/raw_data/{DATA_SET}.jsonl') as f:
        click_stream = ClickStream(f)
    with open(f'{PROJECT_PATH}/clickstream_experiment/data/preprocessed_data/ClickStream_{DATA_SET}.pkl', 'wb') as f:
...
def prepare_file_for_w2v(click_stream, w2v_min_len):
    with open(f'{PROJECT_PATH}/clickstream_experiment/data/preprocessed_data/sequences_{w2v_min_len}.txt', 'w') as f:
        for s in tqdm(click_stream.item_sequences(min_len=w2v_min_len), desc="Writing file for w2v"):
            f.write((" ".join([str(w) for w in s]) + "\n").encode('utf-8'))


def train_w2v(w2v_dim, w2v_epochs, w2v_min_len):
    w2v_model = Word2Vec(sentences=LineSentence(f'{PROJECT_PATH}/clickstream_experiment/data/preprocessed_data/sequences_{w2v_min_len}.txt'),
                         vector_size=w2v_dim, window=5, min_count=1, workers=30, epochs=w2v_epochs)
    w2v_model.wv.save(f'{PROJECT_PATH}/clickstream_experiment/data/preprocessed_data/vectors_train_{w2v_dim}_{w2v_epochs}.kv')


if __name__ == "__main__":
    # logging.debug('Start preprocessing.')
    w2v_dim, w2v_epochs, hmm_nodes, w2v_min_len, hmm_min_len, n_components = parse_args()
