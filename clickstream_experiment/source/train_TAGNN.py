...
from clickstream_experiment.source.tagnn.utils import (
    Data, trans_to_cuda, split_validation, normalize, flatten
)


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_path', default='/ziob/klaudia/FlowHMM/clickstream_experiment/data/preprocessed_data')
    ...
    parser.add_argument('--results_path', type=str, default='/ziob/klaudia/FlowHMM/clickstream_experiment/logs/', help='file to save model results in')
    parser.add_argument('--model_dir', type=str, default='/ziob/klaudia/FlowHMM/clickstream_experiment/data/models', help='directory to save model weights in')
    parser.add_argument(
...
def read_data(opt):
    ...
    train_data = pickle.load(open(f"{data_path}/TAGNN_seq_{opt.min_len}_train.pkl", 'rb'))
    if validation:
