...
class ClickStreamSession(ABC):
    def __init__(self, session, clickstream):
        ...
        self.event_list = [ClickStreamEvent(event, clickstream) for event in session['events'] if len(session['events'])]
...
class ClickStream(ABC):
    def __init__(self, data):
        ...
        self.sessions = [ClickStreamSession(json.loads(session), self) for _, session in enumerate(data)]
