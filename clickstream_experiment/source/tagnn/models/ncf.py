...
def generate_word2vec_embeddings(
    ...
    model = Item2Vec(vector_size=vector_size, **kwargs)
    model.train(data.train, user_col=data.col_user, item_col=data.col_item)

    P_embeddings = model.generate_user_embeddings()
    P_embeddings = P_embeddings.sort_index().values

    Q_embeddings = model.generate_item_embeddings()
    Q_embeddings = Q_embeddings.sort_index().values
...
class NCF:
    ...
    def __init__(
        ...
        init_mlp_embeddings=False,
        init_gmf_embeddings=False,
        freeze_gmf=False,
        combined_gmf=False,
    ):
        """Constructor
        ...
        self.combined_gmf = combined_gmf
        ...
        self._create_model(
            ...
            init_gmf_embeddings=init_gmf_embeddings,
            init_mlp_embeddings=init_mlp_embeddings,
            freeze_gmf=freeze_gmf,
        )
    ...
    def _init_gmf(self, embeddings_data, freeze=False):
        embedding_gmf_P, embedding_gmf_Q = generate_word2vec_embeddings(embeddings_data, self.n_factors)

        self.init_embedding_gmf_P = normalize(embedding_gmf_P)
        self.init_embedding_gmf_Q = normalize(embedding_gmf_Q)

        self.embedding_gmf_P = tf.Variable(
            self.init_embedding_gmf_P,
            name="embedding_gmf_P",
            dtype=tf.float32,
        )

        self.embedding_gmf_Q = tf.Variable(
            self.init_embedding_gmf_Q,
            name="embedding_gmf_Q",
            dtype=tf.float32,
            trainable=not freeze,
        )

    def _init_combined_gmf(self, embeddings_data, freeze=True):
        w2v_size = self.n_factors // 2
        random_size = self.n_factors - w2v_size
        embedding_gmf_P, embedding_gmf_Q = generate_word2vec_embeddings(embeddings_data, w2v_size)

        self.init_embedding_gmf_P = normalize(embedding_gmf_P)
        self.init_embedding_gmf_Q = normalize(embedding_gmf_Q)

        self.embedding_w2v_Q = tf.Variable(
            self.init_embedding_gmf_Q,
            name="embedding_w2v_Q",
            dtype=tf.float32,
            trainable=not freeze,
        )

        self.embedding_random_Q = tf.Variable(
            tf.random.truncated_normal(
                shape=[self.n_items, random_size],
                mean=0.0,
                stddev=0.01,
                seed=self.seed,
            ),
            name="embedding_random_Q",
            dtype=tf.float32,
        )

        self.embedding_gmf_Q = tf.concat([self.embedding_w2v_Q, self.embedding_random_Q], 1)
    ...
    def _init_mlp(self, embeddings_data):
        embedding_mlp_P, embedding_mlp_Q = generate_word2vec_embeddings(embeddings_data, int(self.layer_sizes[0] / 2))

        embedding_mlp_P = normalize(embedding_mlp_P)
        embedding_mlp_Q = normalize(embedding_mlp_Q)
    ...
    def _create_model(
        ...
        with tf.compat.v1.variable_scope("embedding", reuse=tf.compat.v1.AUTO_REUSE):
            ...
            if embeddings_data is not None and init_gmf_embeddings:
                if self.combined_gmf:
                    self._init_combined_gmf(embeddings_data, freeze=freeze_gmf)
                else:
                    self._init_gmf(embeddings_data, freeze=freeze_gmf)
            else:
                self.embedding_gmf_P = tf.Variable(
                    tf.random.truncated_normal(
                        shape=[self.n_users, self.n_factors],
            ...
            if embeddings_data is not None and init_mlp_embeddings:
                self._init_mlp(embeddings_data)
            else:
                self.embedding_mlp_P = tf.Variable(
                    tf.random.truncated_normal(
        ...
        with tf.compat.v1.variable_scope("optimizer_unfrozen", reuse=tf.compat.v1.AUTO_REUSE):
            if self.combined_gmf:
                self.optimizer_unfrozen = tf.compat.v1.train.AdamOptimizer(
                    learning_rate=self.learning_rate
                ).minimize(
                    self.loss,
                    var_list=tf.compat.v1.trainable_variables() + [self.embedding_w2v_Q]
                )
            else:
                self.optimizer_unfrozen = tf.compat.v1.train.AdamOptimizer(
                    learning_rate=self.learning_rate
                    ...
                    self.loss,
                    var_list=tf.compat.v1.trainable_variables() + [self.embedding_gmf_Q, self.embedding_gmf_P]
                )
    ...
    def run_epoch(self, data, unfreeze_gmf=False):
        ...
        for user_input, item_input, labels in data.train_loader(self.batch_size):
            ...
            if not unfreeze_gmf:
                ...
                loss, _ = self.sess.run([self.loss, self.optimizer_unfrozen], feed_dict)
            train_loss.append(loss)

        return train_loss

    def fit(self, data, experiment_name=None, abx_tests_pdf=None, late_init_gmf=None, unfreeze_gmf=None):
        ...
        wandb.init(project="ncf", entity="cirglaboratory", name=experiment_name)
        ...
        log_data = self.evaluate(data, abx_tests_pdf=abx_tests_pdf)
        ...
        for epoch_count in range(1, self.n_epochs+1):
            ...
            if late_init_gmf is not None and epoch_count == late_init_gmf:
                embedding_gmf_P, embedding_gmf_Q = generate_word2vec_embeddings(data, self.n_factors)

                embedding_gmf_P = normalize(embedding_gmf_P)
                embedding_gmf_Q = normalize(embedding_gmf_Q)

                assign_op = self.embedding_gmf_P.assign(embedding_gmf_P)
                self.sess.run(assign_op)

                assign_op = self.embedding_gmf_Q.assign(embedding_gmf_Q)
                self.sess.run(assign_op)

            train_begin = time()

            if unfreeze_gmf is None or epoch_count < unfreeze_gmf:
                train_loss = self.run_epoch(data, unfreeze_gmf=False)
            else:
                train_loss = self.run_epoch(data, unfreeze_gmf=True)

            train_time = time() - train_begin

            # output every self.verbose
            if self.verbose and (epoch_count % self.verbose == 0 or epoch_count == self.n_epochs-1):
                log_data = self.evaluate(data, abx_tests_pdf=abx_tests_pdf)
            else:
                log_data = {}
    ...
    def _predict(self, user_input, item_input, map_indices=True):
    ...
    def evaluate(self, data, k=10, abx_tests_pdf=None, user_col="userID", item_col="itemID"):

        users, items, preds = [], [], []
        item = list(data.train[item_col].unique())
        for user in data.train[user_col].unique():
            user = [user] * len(item)
            users.extend(user)
            items.extend(item)
            preds.extend(list(self._predict(np.array(user), np.array(item), map_indices=False).reshape(-1)))

        all_predictions = pd.DataFrame(data={user_col: users, item_col: items, "prediction": preds})

        merged = pd.merge(data.train, all_predictions, on=[user_col, item_col], how="outer")
        all_predictions = merged[merged.rating.isnull()].drop('rating', axis=1)

        eval_map = map_at_k(data.test, all_predictions, col_prediction='prediction', k=k, col_user=user_col, col_item=item_col)
        eval_ndcg = ndcg_at_k(data.test, all_predictions, col_prediction='prediction', k=k, col_user=user_col, col_item=item_col)
        eval_precision = precision_at_k(data.test, all_predictions, col_prediction='prediction', k=k, col_user=user_col, col_item=item_col)
        eval_recall = recall_at_k(data.test, all_predictions, col_prediction='prediction', k=k, col_user=user_col, col_item=item_col)

        metrics = {
            "MAP": eval_map,
            "NDCG": eval_ndcg,
            f"Precision@{k}": eval_precision,
            f"Recall@{k}": eval_recall,
        }

        if abx_tests_pdf is not None:
            gmf_embeddings = self.embedding_gmf_Q.eval(session=self.sess)
            mlp_embeddings = self.embedding_mlp_Q.eval(session=self.sess)

            gmf_scores = calculate_abx_score(gmf_embeddings, abx_tests_pdf)
            mlp_scores = calculate_abx_score(mlp_embeddings, abx_tests_pdf)

            abx_metrics = {
                'abx_euclidean_gmf': gmf_scores['abx_euclidean'],
                'abx_cosine_gmf': gmf_scores['abx_cosine'],
                'abx_euclidean_mlp': mlp_scores['abx_euclidean'],
                'abx_cosine_mlp': mlp_scores['abx_cosine'],
            }

            if self.combined_gmf:
                gmf_w2v = self.embedding_w2v_Q.eval(session=self.sess)
                gmf_random = self.embedding_random_Q.eval(session=self.sess)

                w2v_scores = calculate_abx_score(gmf_w2v, abx_tests_pdf)
                random_scores = calculate_abx_score(gmf_random, abx_tests_pdf)

                abx_metrics['abx_euclidean_w2v'] = w2v_scores['abx_euclidean']
                abx_metrics['abx_cosine_w2v'] = w2v_scores['abx_cosine']
                abx_metrics['abx_euclidean_random'] = random_scores['abx_euclidean']
                abx_metrics['abx_cosine_random'] = random_scores['abx_cosine']

            metrics = {**metrics, **abx_metrics}
