...
try:
    ...
    from data.bert4rec_yoochoose_datasets import YooChoosePreprocessor, get_raw_data
...
try:
    ...
    from .data.bert4rec_yoochoose_datasets import (  # noqa F811
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--test_only",
        action='store_true',
        help="only use test set, not validation",
    )
    parser.add_argument(
        "--name",
        type=str,
    ...
    with torch.no_grad():
        with open(results_path, 'w') as out_file:
            for _, batch in enumerate(val_loader):
                ...
                _save_batch(model, batch, device, out_file, smap)
...
def _save_batch(
    ...
    _, cuts = torch.sort(-scores, dim=1)
    for cands, cut in zip(candidates, cuts):
        cut = cands[cut[:20]]
        cut = [smap[item] for item in cut.cpu().numpy()]
        out_file.write(f'{cut}\n')
...
def _validate(
    ...
    outputs = [None for _ in range(dist.get_world_size())]
    if 20 in metric_ks:
        keys = ["Recall@1", "Recall@5", "Recall@10", "Recall@20", "NDCG@20"]
    else:
        keys = ["Recall@1", "Recall@5", "Recall@10"]
...
def train_val_test(
    ...
    best_recall = 0
    if test_only:
        _validate(model, test_loader, device, -1, metric_ks, True)
    else:
    ...
    for epoch in range(num_epochs):
        ...
        if test_only:
            metrics = _validate(model, test_loader, device, epoch, metric_ks, True)
        else:
            metrics = _validate(model, val_loader, device, epoch, metric_ks)

        if 20 in metric_ks:
            score = metrics['Recall@20']
        else:
            score = metrics['Recall@10']
        if best_recall < score:
            best_recall = score
            if results_path is not None:
                _save_results(model, test_loader, device, results_path, smap)
        ...
        if dist.get_rank() == 0:
            ...
            print(
                ...
                f"{'Epoch ' + str(epoch + 1)}, metrics {metrics}"
            )

        if export_root is not None and (epoch + 1) % 10 == 0:
            torch.save(
                model.state_dict(),
                export_root + f"epoch_{epoch + 1}_model.pth",
            )
            print(f"epoch {epoch + 1} model has been saved to {export_root}")
    metrics = _validate(model, test_loader, device, num_epochs, metric_ks, True)
    if dist.get_rank() == 0:
        wandb.log(metrics)
        print(
...
def main(argv: List[str]) -> None:
    ...
    if dist.get_rank() == 0:
        wandb.init(project="bert4rec", entity="cirglaboratory", name=args.name, config=args)

    if args.dataset_name == 'yoochoose':
        train_sequences, test_data = get_raw_data(
            args.dataset_path
        )

        df = YooChoosePreprocessor(
            train_sequences,
            test_data,
            args.max_len,
            args.mask_prob,
