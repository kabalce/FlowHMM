...
from clickstream_experiment.source.clickstream import ClickStream, zero_init
...
def train_w2v(w2v_dim, w2v_epochs, w2v_min_len, min_count):
    w2v_model = Word2Vec(
    ...
    w2v_model.wv.save(
        f"{PROJECT_PATH}/clickstream_experiment/data/preprocessed_data/vectors_train_{w2v_dim}_{w2v_min_len}_{w2v_epochs}_{min_count}.kv"
    )


def tidy_up(cs, cs_path_cleaned):
    # znajdź rzadkie produkty
    freqs = pd.Series(cs.item_ids).sort_values()
...
if __name__ == "__main__":
    ...
    cs_path_cleaned = f"{PROJECT_PATH}/clickstream_experiment/data/preprocessed_data/ClickStream_{DATA_SET}_cleaned.pkl"
    w2v_path = f"{PROJECT_PATH}/clickstream_experiment/data/preprocessed_data/sequences_{w2v_min_len}_cleaned.txt"
