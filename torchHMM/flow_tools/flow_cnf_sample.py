...
def ParseArguments():
    ...
    parser.add_argument('--n', default="1000", required=False, help='nr of   data to sample from flow (default: %(default)s)')

    parser.add_argument('--input-model', default="flow_model.pt", required=False,
                        help='input model (default: %(default)s)')
    parser.add_argument('--input-train-data', default="train_data.pkl", required=False,
...
if(data_dim==2):
    ...
    plot_2D_simple(samples_flow.detach().cpu().numpy(), title="Data sampled from flow, v2")
