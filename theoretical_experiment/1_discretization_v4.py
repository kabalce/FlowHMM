...
results_path = f"{PROJECT_PATH}/theoretical_experiment/1_results_v4"
...
if __name__ == "__main__":
    ...
    for discretize_meth in DISCRETIZATION_TECHNIQUES[-3:]:
        for n in grid_sizes:
            ...
            plot_HMM(  # Example plot
                ...
                discretize_meth,
                n,
                f"{results_path}/1_nodes_{discretize_meth}_{n}_v4.png",
            )
            ...
            plot_Qs(Q_from_params(model), model._cooccurence(model.discretize(X_train, True)), f"{results_path}/1_Q_{discretize_meth}_{n}_v4.png")


    with open(
        f"{results_path}/1_discretization_v4.json",
        "w",
    ) as f:
        json.dump(results, f)
    ...
    for metric, title in zip(['d_tv', 'kl', 'acc', 'll'], ["Total variation distance", "KL divergence", 'State prediction accuracy', 'Loglikelihood']):
        plot_metric(results, metric, title, f"{results_path}/1_{metric}_v4.png")
