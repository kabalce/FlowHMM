...
def plot_HMM(X, model, discretize_meth, n, path=None):
    ...
    x1, y1 = X.min(axis=0) - 0.5
    x2, y2 = X.max(axis=0) + 0.5
...
def plot_Qs(Q_cooc, Q_true_model, path):
    ...
    vmin, vmax = min(Q_cooc.min(), Q_true_model.min()), max(
        Q_cooc.max(), Q_true_model.max()
    )
    ...
    plt.savefig(
        path
    )  # f"{PROJECT_PATH}/theoretical_experiment/plots/1_Q_{discretize_meth}_{n}_v2.png"
    plt.show()
    plt.close()
...
def plot_HMM3(X, model, path=None):
    ...
    # TODO: transformuj dane data siecią
    lls = (
        model.model.emission_matrix(torch.tensor(data).to(model.model.device).float())[
            0
        ]
        .cpu()
        .detach()
