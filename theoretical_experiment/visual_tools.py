...
def plot_HMM3(X, model, path=None):
    ...
    data = np.column_stack((XX.ravel(), YY.ravel()))
    # TODO: transformuj dane data siecią
    lls = np.concatenate([model.model.emission_score(model.model.NFs[i], torch.Tensor(data).to(model.model.device)).detach().numpy().reshape(-1, 1) for i in range(model.n_components)], axis=1)
