...
def init_params(X_expert, Z_expert):
    ...
    means = np.concatenate(
        [X_expert[Z_expert == i, :].mean(axis=0).reshape(1, -1) for i in range(n_comp)]
    )
    stds = np.concatenate(
        [X_expert[Z_expert == i, :].std(axis=0).reshape(1, -1) for i in range(n_comp)]
    )
...
if __name__ == "__main__":
    ...
    for _ in tqdm(range(20)):  # the initialization differ in runs
        ...
        model.covars_ = np.concatenate(
            [np.diag(x)[np.newaxis, :, :] for x in covars_init], axis=0
        )
        ...
        for n in grid_sizes:
            ...
            for max_epoch, lr in itertools.product([100, 200], [0.001, 0.003, 0.01]):
                ...
                plot_HMM3(
                    X_test,
                    model,
                    path=f"{results_path}/3_dist_on_moons_{discretize_meth}_{n}_{max_epoch}_{lr}.png",
                )
                plot_Qs(
                    Q_from_params(model),
                    model._cooccurence(model.discretize(X_train, True)),
                    f"{results_path}/3_Q_{discretize_meth}_{n}_{max_epoch}_{lr}.png",
                )
